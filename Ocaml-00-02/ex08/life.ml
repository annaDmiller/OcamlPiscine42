...
let helix_of_string (orig : string) : helix =
    let rec aux (ind : int) : helix =
        if ind >= String.length orig then
            []
        else
            generate_nucleotide orig.[ind] :: aux (ind + 1)
    in
    aux 0

let generate_life (orig : string) : unit =
    let value_helix = helix_of_string orig in
    let comp_helix = complementary_helix value_helix in
    let m_rna = generate_rna value_helix in
    let prot = decode_arn m_rna 
    in
    print_endline "====== The Complete Process of Protein Creation ======";
    print_string "Input DNA     : ";
    print_endline orig;
    print_string "Helix         : ";
    print_endline (helix_to_string value_helix);
    ...
    print_endline (protein_to_string prot);
    print_newline ()
...
let () =
  generate_life "ATGCGTACGTTAGC";
  generate_life "ATGAAATAG";
  generate_life (String.concat "" [ "ATG"; "GCC"; "TTT"; "TAA" ])
