...
let newDeck () : t =
    let rec shuffle (rest : t) : t =
        match rest with
            | [] -> []
            | _ -> (
                let ind = Random.int (List.length rest)
                in
                let rec pick (ind : int) (lst : t) : Card.t * t =
                    match lst with
                        | [] -> raise (invalid_arg "Deck.pick")
                        | x :: xs -> (
                            if ind = 0 then
                                (x, xs)
                            else
                                let (card, rest) = pick (ind - 1) xs
                                in
                                (card, x :: rest)
                        )
                in
                let (card, rest) = pick ind rest
                in
                card :: shuffle rest
            )
    in
...
let drawCard (deck : t) : Card.t * t =
    match deck with
        | [] -> raise (Failure "Deck.drawCard: cannot draw a card from an empty deck")
        | card :: rest -> (card, rest)